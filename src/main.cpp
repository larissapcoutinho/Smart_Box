...
// ponteiros para vetores, em seguida, alocar memória para eles
int *alarmHours;
int *alarmMin;
...
Keypad keypad(makeKeymap(teclas), pinos_linhas, pinos_colunas, LINHAS, COLUNAS);
...
void task_menu(void *pvParameters)
{
    while (1)
    {
        ...
        if (tecla != NO_KEY) // Quando a tecla for pressionada
        {
            lcd.setCursor(15, 1);
            lcd.print(tecla);
    ...
    while (flag2)
    {
        ...
        if (tecla != NO_KEY)
        {
            ...
            default:
                ...
                if (pos < maxAlarms)
                {
                    alarmHours[pos] = (alarmHours[pos] % 10) * 10 + (tecla - '0');
                    lcd.setCursor(14, 1);
    ...
    while (flag3)
    {
        ...
        if (tecla != NO_KEY)
        {
            ...
            default:
                if (pos < maxAlarms)
                {
                    alarmMin[pos] = (alarmMin[pos] % 10) * 10 + (tecla - '0');
                    lcd.setCursor(14, 1);
...
void task_menu(void *pvParameters)
{
    while (1)
    {
        if (WiFi.status() == WL_CONNECTED)
        {
            ...
            lcd.setCursor(0, 1);
            lcd.print(timeinf.tm_hour);
            lcd.setCursor(2, 1);
            lcd.print(":");
            lcd.print(timeinf.tm_min);
            lcd.setCursor(5, 1);
            lcd.print(":");
            lcd.print(timeinf.tm_sec);
